...
combined_raw_audio_information_array_BE = np.zeros(len(raw_audio_information_1_array_BE), np.int16);
...
while j < len(combined_raw_audio_information_array_BE):
    combined_raw_audio_information_array_BE[j] = combined_raw_audio_information_array_BE[j] + 0.5 * raw_audio_information_1_array_BE[j] + 0.5 * raw_audio_information_2_array_BE[j];
...
with wave.open("Combined.wav", "wb") as wf_3:
    p = pyaudio.PyAudio();
